...
func generateContent(mcList []llms.MessageContent) (*llms.ContentResponse, error) {
...
var chatHistory = memory.NewConversationBuffer(memory.WithChatHistory(buffChatMemory))
...
func langchainRun(c *ava.Context, uid, content, deviceList, commands string) (*aiResp, error) {
	...
	c.Debugf("FROM |chains.Run |result=%s |resp=%v", out, resp)
