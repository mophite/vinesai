...
func (e *Executor) Call(ctx context.Context, inputValues map[string]any, _ ...chains.ChainCallOption) (map[string]any, error) { //nolint:lll
	...
	for i := 0; i < e.MaxIterations; i++ {
		...
		steps, finish, observation, err = e.doIteration(ctx, steps, nameToTool, inputs)
	...
	return e.getReturn(
		&schema.AgentFinish{ReturnValues: map[string]any{"output": observation}},
		steps,
	), ErrNotFinished
}
...
func (e *Executor) doIteration( // nolint
...
) ([]schema.AgentStep, map[string]any, string, error) {
	...
	for _, action := range actions {
		steps, observation, err = e.doAction(ctx, steps, nameToTool, action)
		if err != nil {
			return steps, nil, observation, err
		}
	}

	return steps, nil, observation, nil
}
